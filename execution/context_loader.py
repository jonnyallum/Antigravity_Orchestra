...
Built by @Manus (Mason Drake "The Bridgemaster") for AgOS 4.0.

Quick-loads relevant project context, recent changes, active blockers,
and key files into any session for fast onboarding.
Integrated with Shared Brain for cross-agent learning synchronization.
...
def get_shared_brain_learnings() -> list:
    """Fetch recent learnings from the local Shared Brain cache."""
    learnings = []
        ...
        for agent_file in LEARNINGS_DIR.glob("*.json"):
            try:
                with open(agent_file, 'r', encoding='utf-8') as f:
                    data = json.load(f)
                agent_learnings = data.get("learnings", [])
                # Get last 3 learnings from each agent
                for l in agent_learnings[-3:]:
                    l["agent"] = agent_file.stem
                    learnings.append(l)
            except:
                pass
...
def load_project_context(project_path: Path) -> dict:
    ...
    context = {
        ...
        "shared_brain": get_shared_brain_learnings()
    }
